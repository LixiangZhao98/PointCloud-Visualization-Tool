	...
	SubShader{
        ...
        float _CustomRadius;
		  ...
		  StructuredBuffer<Triangle> triangleRW;
			...
			//	float3 position = triangleRW[vertex_id/3].v[vertex_id%3].vPosition+normalize(triangleRW[vertex_id/3].v[vertex_id%3].vNormal)*_CustomRadius;

				//float3 position = triangleRW[vertex_id/3].v[vertex_id%3].vPosition+normalize((triangleRW[vertex_id/3].v[0].vNormal+triangleRW[vertex_id/3].v[1].vNormal+triangleRW[vertex_id/3].v[2].vNormal)/3)*_CustomRadius;
...
// 		    unity_ObjectToWorld = _LocalToWorld;
//             unity_WorldToObject = _WorldToLocal;
//             o.position=mul(UNITY_MATRIX_VP,  mul(unity_ObjectToWorld, float4(position, 1)));
//             o.worldPos= mul(unity_ObjectToWorld, float4(position, 1));
//             float3 worldNormal = mul(triangleRW[vertex_id/3].v[vertex_id%3].vNormal, (float3x3)unity_WorldToObject);
...
// 				return fixed4(_Color.x,_Color.y,_Color.z,_Color.a*viewDotNorm) ;
// 			}
