...
public class GPUKDECsHelper
{
    ...
    static public void StartGpuKDE(ParticleGroup pG, DensityField dF, ComputeShader KDE_cs_)
    {
        ...
        int kernel_aveDen = KDE_Cs.FindKernel("aveDen");
        ...
        float sum_ = 0f;
        foreach (var n in dens)
        {
            sum_ += n;
        }
        totalDen = new ComputeBuffer(1, sizeof(float));
        float[] tD = new float[1] { sum_ };
        totalDen.SetData(tD);
        ...
        KDE_Cs.SetBuffer(kernel_SL_Modified, "totalDen", totalDen);
        ...
        KDE_Cs.SetVector("gridStep", new Vector4(dF.XSTEP, dF.YSTEP, dF.ZSTEP, 0f));
        KDE_Cs.SetVector("gridNum", new Vector4(dF.XNUM, dF.YNUM, dF.ZNUM, 0f));
        KDE_Cs.Dispatch(kernel_SL_Modified, dF.XNUM / 8, dF.YNUM / 8, dF.ZNUM / 8);//new SL
        ...
        AddLUT(pG, dF);
        Debug.Log("Finish density estimation on GPU; Finish gradient estimation on GPU; Finish particle density and SL estimation on GPU; Finish Adding particles to LUT on GPU");
